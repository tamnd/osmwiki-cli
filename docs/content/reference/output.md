...
describes what users get. Pick a format with `-o`, or let osmwiki choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
osmwiki <command> -o table   # aligned columns for reading
osmwiki <command> -o jsonl   # one JSON object per line, for piping
osmwiki <command> -o json    # a single JSON array
osmwiki <command> -o csv     # spreadsheet friendly
osmwiki <command> -o tsv     # tab-separated
osmwiki <command> -o url     # just the URL column
osmwiki <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
osmwiki <command> --fields id,title,url
```
...
```bash
osmwiki <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
osmwiki <command>            # a table, because this is a terminal
osmwiki <command> | wc -l    # JSONL, because this is a pipe
```
